...
Book* searchBookByTitle(Book* root, const char* title, Book* result) {
    if (root == NULL) {
        return result;
    }
    ...
    result = searchBookByTitle(root->left, title, result);
    result = searchBookByTitle(root->right, title, result);
...
void addUser() {
    ...
    userCount++;
    User* newUser = createUser(userCount, name, user_id, age, gender);
...
BorrowRecord* createBorrowRecord(int userId, int bookId) {
    ...
    newRecord->dueDate = newRecord->borrowDate + (14 * 24 * 60 * 60); // 14 days loan period
...
void manageBorrowing() {
    ...
    do {
        ...
        printf("8. Undo Last Return\n");
        printf("9. View All Borrow Records\n");
        printf("10. Back\n");
        printf("Choose an option: ");
        scanf("%d", &choice);
